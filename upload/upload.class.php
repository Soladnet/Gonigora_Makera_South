    ...
    protected function get_file_object($file_name) {
        ...
        if (is_file($file_path) && $file_name[0] !== '.') {
            ...
            foreach ($this->options['image_versions'] as $version => $options) {
                ...
                if (is_file($options['upload_dir'] . $imgName)) {
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                $file->url = $this->options['upload_url'] . rawurlencode($file->name);
                foreach ($this->options['image_versions'] as $version => $options) {
                    $imgext = strtolower(substr(strrchr(rawurlencode($file->name), '.'), 1));
                    $imgName = substr(rawurlencode($file->name), 0, strrpos(rawurlencode($file->name), '.')) . '_' . $options['max_width'] . 'x' . $options['max_width'] . '.' . $imgext;
                    if ($this->create_scaled_image($file->name, $options)) {
                        if ($this->options['upload_dir'] !== $options['upload_dir']) {
                            $file->{$version . '_url'} = $options['upload_url'] . $imgName;
                        } else {
                            clearstatcache();
                            $file_size = filesize($file_path);
                        }
    ...
    public function delete() {
        ...
        if ($success) {
            foreach ($this->options['image_versions'] as $version => $options) {
                $imgext = strtolower(substr(strrchr($file_name, '.'), 1));
                $imgName = substr($file_name, 0, strrpos($file_name, '.')) . '_' . $options['max_width'] . 'x' . $options['max_width'] . '.' . $imgext;
                $file = $options['upload_dir'] . $imgName;
                if (is_file($file)) {
                    unlink($file);
                }
            }
